...
const getMotionDuration = (distance: number) =>
  Math.min(900, Math.max(420, distance * 7));
...
function TOCItem({ item, active }: { item: TOCItemDef; active: boolean }) {
...
function ActiveTocPath({
  ...
  const updateThumb = useCallback(() => {
    ...
    if (!element || !start || !end || !startLength || !endLength) return;
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < computed.positions.length; i++) {
      ...
      let length =
        i > 0 ? lengths[i - 1][1] + (top - computed.positions[i - 1][1]) : 0;
  ...
  if (!activeStartPosition || !activeEndPosition) return null;
...
function getActiveStartIndex(items: TOCItemDef[], activeIndex: number) {
  ...
  if (!active || active.depth <= 2) return activeIndex;
  for (let i = activeIndex - 1; i >= 0; i--) {
    if (items[i].depth < active.depth) return i;
  }
  return activeIndex;
}
